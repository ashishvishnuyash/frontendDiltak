...
export const EarlyWarnings: React.FC<EarlyWarningsProps> = ({ data, loading }) => {
  ...
  // Limit to 10 alerts max for performance
  const visibleAlerts = data.alerts.slice(0, 10);
  const hasMoreAlerts = data.alerts.length > 10;

  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5 h-[380px] flex flex-col">
        <div className="flex items-center justify-between mb-4 shrink-0">
          <h2 className="text-base font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="max-w-[240px] text-xs p-3 rounded-xl">
                  AI-modeled signals across the entire cohort. No attribution to individuals. Max 10 active alerts shown.
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </h2>
          <div className="flex items-center gap-2">
            ...
            {data.alerts.length > 0 && (
              <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">
              ...
              {visibleAlerts.map((alert, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 leading-snug line-clamp-2">{alert.description}</p>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">{alert.period}</p>
                  </div>
                  <ArrowUpRight className="h-3.5 w-3.5 text-gray-300 group-hover:text-gray-500 dark:group-hover:text-gray-400 transition-colors shrink-0 mt-0.5" />
                </motion.div>
              ...
              {hasMoreAlerts && (
                <div className="text-center py-2">
                  <p className="text-[10px] text-gray-400 dark:text-gray-500">
                    +{data.alerts.length - 10} more alerts not shown
                  </p>
                </div>
              )}
            </>
          ) : (
            <div className="flex flex-col items-center justify-center h-full gap-2 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700">
              <ShieldCheck className="h-8 w-8 text-emerald-300 dark:text-emerald-700" />
              <p className="text-xs text-gray-400 dark:text-gray-500 font-medium">No Active Warnings</p>
            </div>
          )}
