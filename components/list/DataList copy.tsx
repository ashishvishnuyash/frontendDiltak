...
export interface DataListProps<T> {
  ...
  /** Show import button */
  is_import?: boolean;
  /** API endpoint to POST imported file */
  create_api?: string;
...
function ImportModal<T extends Record<string, any>>({
  ...
  async function downloadTemplate() {
    ...
    try {
      // If template_api is provided, fetch from backend
      if (template_api) {
        const token = localStorage.getItem('access_token');
  ...
  async function processFile(file: File) {
    ...
    try {
      ...
      // Append file - backend expects field name 'file'
      // If your backend expects a different field name, change this line
      ...
      if (response.data.success || response.status === 200 || response.status === 201) {
        // Try to get count from response in various formats
        let count = 0;
        if (response.data.count) {
          ...
          count = 1; // Default if count not provided
        }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Check for success and data property
