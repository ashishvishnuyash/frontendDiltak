...
//       const responseData: DeleteEmployeeResponse = await res.json();

//       if (!res.ok || !responseData.success) {
//         throw new Error(
//           responseData.message || `Delete failed with status ${res.status}`,
//         );
//       }
...
//         return (
//           <span
//             className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest ${val !== false ? "text-emerald-600" : "text-gray-400"}`}
//           >
//             <span
//               className={`w-1.5 h-1.5 rounded-full ${val !== false ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-gray-400"}`}
//             />
...
//                 (e) => e.isActive !== false && e.is_active !== false,
//               ).length,
...
//                   e.department ?? "",
//                   e.position ?? "",
//                   e.isActive !== false && e.is_active !== false
//                     ? "Active"
//                     : "Inactive",
//                 ]),
//               ]
...
function EmployeesPage() {
  ...
  const template_columns: ColumnDef<Employee>[] = [
    {
      key: "firstName",
      title: "firstName",
    },
      ...
      <div className="px-4 sm:px-6 lg:px-6 py-6 max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm">
          <DataList<Employee>
            key={refreshKey}
            apiPath={`${ServerAddress}/employees`}
            create_api={`${ServerAddress}/employees/bulk-create`}
